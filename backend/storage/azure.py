...
class AzureBlobStorageBackend(StorageBackend):
    ...
    async def read_bytes(self, path: str) -> bytes:
        """Download blob body (full read). Used for server-side processing (e.g. bulk applicability)."""
        if self._bypass:
            raise OSError(
                "Cannot read blobs when BYPASS_AZURE_UPLOAD is enabled; disable bypass or use real Azure storage."
            )
        try:
            container = await self._get_container()
            blob = container.get_blob_client(path)
