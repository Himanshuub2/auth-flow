...
class Event(BaseEvents):
    __tablename__ = events_table("events")
    ...
    event_dates: Mapped[dict | None] = mapped_column(json_type(), nullable=True)
    ...
    applicability_type: Mapped[ApplicabilityType] = mapped_column(
        Enum(ApplicabilityType, name="applicability_type", schema=schema_events(), create_constraint=True),
    ...
    replaces_document_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey(fk_events("events"), ondelete="SET NULL"), nullable=True
    )
...
class EventRevision(BaseEvents):
    ...
    __tablename__ = events_table("event_revisions")
    __table_args__ = (
