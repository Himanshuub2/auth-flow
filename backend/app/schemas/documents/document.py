...
from app.models.documents.document_file import DocumentFileType


class DocumentSavePayload(BaseModel):
    name: str
    document_type: DocumentType  # FE sends display label (e.g. "Policy"); validator converts to enum
    tags: list[str]
    ...
    def document_type_to_enum(cls, v: str | DocumentType) -> DocumentType:
        if isinstance(v, DocumentType):
            return v
        s = (v or "").strip()
        # Accept enum constant (e.g. POLICY, LATEST_NEWS_AND_ANNOUNCEMENTS)
        try:
            return DocumentType(s)
        except ValueError:
            pass
        # Accept display label (e.g. Policy, Latest News & Announcements)
...
class DocumentOut(BaseModel):
    id: int
    name: str
    document_type: str  # Display label (e.g. "Policy")
    tags: list | None
    summary: str | None
...
class DocumentRevisionOut(BaseModel):
    ...
    version_display: str
    name: str
    document_type: str  # Display label
    tags: list | None
    summary: str | None
    applicability_type: ApplicabilityType
    applicability_refs: dict | None
